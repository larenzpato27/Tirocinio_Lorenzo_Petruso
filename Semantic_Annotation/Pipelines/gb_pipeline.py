...
def main():
    ...
    output_dir = 'Results_GradientBoosting_XXXXXXXXXX'
    ...
    # 4. Preparazione modello gradient boosting e validazioni
    ...
    print("\n" + "=" * 50)
    print("\n 1: Gradient Boosting - Standard 10-Fold (senza contesto)...")
    y_pred_kf_senza = np.empty_like(y)
    fold = 1
    for train_index, test_index in kf.split(X_Senza, y):
        print(f"   Esecuzione Fold {fold}/10 (Standard)...")
        gb_model.fit(X_Senza[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_kf_senza[test_index] = gb_model.predict(X_Senza[test_index])
        fold += 1
    ...
    plt.title('GB - Standard 10-Fold (No Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_GB_Standard_NoContext.png'), dpi=300)
    plt.close()

    # Fase 2: stratified 10-fold (senza contesto)
    print("\n" + "=" * 50)
    print("\n 2: Gradient Boosting - Stratified 10-Fold (senza contesto)...")
    ...
    for train_index, test_index in skf.split(X_Senza, y):
        ...
        gb_model.fit(X_Senza[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_skf_senza[test_index] = gb_model.predict(X_Senza[test_index])
        fold += 1
    ...
    sns.heatmap(confusion_matrix(y, y_pred_skf_senza, labels=np.unique(y)), annot=True, fmt='d', cmap='Oranges',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('GB - Stratified 10-Fold (No Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_GB_Stratified_NoContext.png'), dpi=300)
    plt.close()

    # Fase 3: standard 10-fold (con contesto)
    print("\n" + "=" * 50)
    print("\n 3: Gradient Boosting - Standard 10-Fold (con contesto)...")
    ...
    for train_index, test_index in kf.split(X_Con, y):
        ...
        gb_model.fit(X_Con[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_kf_con[test_index] = gb_model.predict(X_Con[test_index])
        fold += 1
    ...
    sns.heatmap(confusion_matrix(y, y_pred_kf_con, labels=np.unique(y)), annot=True, fmt='d', cmap='Oranges',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('GB - Standard 10-Fold (With Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_GB_Standard_Context.png'), dpi=300)
    plt.close()

    # Fase 4: stratified 10-fold (con contesto)
    print("\n" + "=" * 50)
    print("\n 4: Gradient Boosting - Stratified 10-Fold (con contesto)...")
    ...
    for train_index, test_index in skf.split(X_Con, y):
        ...
        gb_model.fit(X_Con[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_skf_con[test_index] = gb_model.predict(X_Con[test_index])
        fold += 1
    ...
    sns.heatmap(confusion_matrix(y, y_pred_skf_con, labels=np.unique(y)), annot=True, fmt='d', cmap='Oranges',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('GB - Stratified 10-Fold (With Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_GB_Stratified_Context.png'), dpi=300)
    plt.close()

    # 5. Salvataggio report
    # Sostituire il nome del file con l'embedder utilizzato: BGELarge - MPNET- MiniL6
    with open(os.path.join(output_dir, 'classification_reports_GB_XXXXXXXXXX.txt'), 'w') as f:
        f.write("=== [FASE 1] GB STANDARD K-FOLD (NO CONTEXT) ===\n")
        f.write(report_kf_senza)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 2] GB STRATIFIED K-FOLD (NO CONTEXT) ===\n")
        f.write(report_skf_senza)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 3] GB STANDARD K-FOLD (WITH CONTEXT) ===\n")
        f.write(report_kf_con)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 4] GB STRATIFIED K-FOLD (WITH CONTEXT) ===\n")
