...
def main():
    ...
    output_dir = 'Results_NB_XXXXXXXXXX'
    ...
    # Calcolo dei pesi per gestire lo sbilanciamento delle classi
    sample_weights = compute_sample_weight(class_weight='balanced', y=y)

    # 4. Preparazione modello naive bayes e validazioni
    nb_model = GaussianNB()
    ...
    print("\n" + "=" * 50)
    print("\n 1: Naive Bayes - Standard 10-Fold (senza contesto)...")
    y_pred_kf_senza = np.empty_like(y)
    fold = 1
    for train_index, test_index in kf.split(X_Senza, y):
        print(f"   Esecuzione Fold {fold}/10 (Standard)...")
        nb_model.fit(X_Senza[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_kf_senza[test_index] = nb_model.predict(X_Senza[test_index])
        fold += 1
    report_kf_senza = classification_report(y, y_pred_kf_senza)

    plt.figure(figsize=(12, 8))
    sns.heatmap(confusion_matrix(y, y_pred_kf_senza, labels=np.unique(y)), annot=True, fmt='d', cmap='Reds',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('NB - Standard 10-Fold (No Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_NB_Standard_NoContext.png'), dpi=300)
    plt.close()

    # Fase 2: stratified 10-fold (senza contesto)
    print("\n" + "=" * 50)
    print("\n 2: Naive Bayes - Stratified 10-Fold (senza contesto)...")
    ...
    for train_index, test_index in skf.split(X_Senza, y):
        ...
        nb_model.fit(X_Senza[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_skf_senza[test_index] = nb_model.predict(X_Senza[test_index])
        fold += 1
    report_skf_senza = classification_report(y, y_pred_skf_senza)

    plt.figure(figsize=(12, 8))
    sns.heatmap(confusion_matrix(y, y_pred_skf_senza, labels=np.unique(y)), annot=True, fmt='d', cmap='Reds',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('NB - Stratified 10-Fold (No Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_NB_Stratified_NoContext.png'), dpi=300)
    plt.close()

    # Fase 3: standard 10-fold (con contesto)
    print("\n" + "=" * 50)
    print("\n 3: Naive Bayes - Standard 10-Fold (con contesto)...")
    ...
    for train_index, test_index in kf.split(X_Con, y):
        ...
        nb_model.fit(X_Con[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_kf_con[test_index] = nb_model.predict(X_Con[test_index])
        fold += 1
    report_kf_con = classification_report(y, y_pred_kf_con)

    plt.figure(figsize=(12, 8))
    sns.heatmap(confusion_matrix(y, y_pred_kf_con, labels=np.unique(y)), annot=True, fmt='d', cmap='Reds',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('NB - Standard 10-Fold (With Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_NB_Standard_Context.png'), dpi=300)
    plt.close()

    # Fase 4: stratified 10-fold (con contesto)
    print("\n" + "=" * 50)
    print("\n 4: Naive Bayes - Stratified 10-Fold (con contesto)...")
    ...
    for train_index, test_index in skf.split(X_Con, y):
        ...
        nb_model.fit(X_Con[train_index], y[train_index], sample_weight=sample_weights[train_index])
        y_pred_skf_con[test_index] = nb_model.predict(X_Con[test_index])
        fold += 1
    report_skf_con = classification_report(y, y_pred_skf_con)

    plt.figure(figsize=(12, 8))
    sns.heatmap(confusion_matrix(y, y_pred_skf_con, labels=np.unique(y)), annot=True, fmt='d', cmap='Reds',
                xticklabels=np.unique(y), yticklabels=np.unique(y))
    plt.title('NB - Stratified 10-Fold (With Context)')
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'confusion_matrix_NB_Stratified_Context.png'), dpi=300)
    plt.close()

    # 5. Salvataggio report
    # Sostituire il nome del file con l'embedder utilizzato: BGELarge - MPNET- MiniL6
    with open(os.path.join(output_dir, 'classification_reports_NB_XXXXXXXXXXX.txt'), 'w') as f:
        f.write("=== [FASE 1] NB STANDARD K-FOLD (NO CONTEXT) ===\n")
        f.write(report_kf_senza)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 2] NB STRATIFIED K-FOLD (NO CONTEXT) ===\n")
        f.write(report_skf_senza)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 3] NB STANDARD K-FOLD (WITH CONTEXT) ===\n")
        f.write(report_kf_con)
        f.write("\n\n==================================================\n")
        f.write("=== [FASE 4] NB STRATIFIED K-FOLD (WITH CONTEXT) ===\n")
